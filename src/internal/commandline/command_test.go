...
func TestParseCommandOK(t *testing.T) {
	cases := []struct {
		...
		{[]string{"c"}, Command{Kind: CommandRuntime, RuntimeMode: mode.Client, RequiresElevation: true}},
		{[]string{"s"}, Command{Kind: CommandRuntime, RuntimeMode: mode.Server, RequiresElevation: true}},
		{[]string{"  c  "}, Command{Kind: CommandRuntime, RuntimeMode: mode.Client, RequiresElevation: true}},
...
func TestRuntimeModeArgs(t *testing.T) {
	got, err := RuntimeModeArgs(mode.Server)
	if err != nil {
	...
	got, err = RuntimeModeArgs(mode.Client)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
