...
func (s Manager) Create(connSettings settings.Settings) (io.ReadWriteCloser, error) {
	...
	if configureErr := s.firewall.configure(tunName, extIface, connSettings, ipv4, ipv6); configureErr != nil {
		_ = tunFile.Close()
		if cleanupErr := s.Remove(connSettings); cleanupErr != nil {
...
func (s Manager) Remove(connSettings settings.Settings) error {
