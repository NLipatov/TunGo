...
func (m *TunFactoryMockIP) RouteAddSplitDev(_ string, _ []string) error      { return nil }
func (m *TunFactoryMockIP) Route6AddSplitDev(_ string, _ []string) error     { return nil }
...
type TunFactoryMockIPTBenign struct{}
...
func TestCreateAndDispose_SuccessAndSkipForwardingDisableWhenExtIfaceUnknown(t *testing.T) {
	...
	if err := f.Remove(cfg); err != nil {
		t.Fatalf("Remove: %v", err)
	}
}

func TestRemove_NoSuchInterface_IsBenign_NoError(t *testing.T) {
	...
	if err := f.Remove(cfg); err != nil {
		t.Fatalf("Remove should ignore missing iface: %v", err)
	}
...
func TestCreate_RejectsLegacyIPv6InIPv4SubnetField(t *testing.T) {
...
func TestCreate_ConfigureError_TriggersCleanup(t *testing.T) {
	ipMock := &TunFactoryMockIP{}
	...
	// Remove deletes the interface only if it exists on the host.
...
func TestDisposeTunDevices_DeleteError(t *testing.T) {
	...
	if err := f.Remove(cfg); err == nil || !strings.Contains(err.Error(), "error deleting TUN device") {
...
func TestRemove_BenignIptablesErrorsAreIgnored(t *testing.T) {
	...
	// Act + Assert
	if err := f.Remove(cfg); err != nil {
		t.Fatalf("Remove should ignore benign iptables errors, got: %v", err)
	}
}

func TestRemove_NonBenignIptablesErrorsAreLoggedButIgnored(t *testing.T) {
	...
	// Act + Assert
	if err := f.Remove(cfg); err != nil {
		t.Fatalf("Remove should not fail on non-benign iptables errors (only log), got: %v", err)
	}
}
...
func TestEnableForwarding_WritesWhenDisabled_Succeeds(t *testing.T) {
	...
	tun, err := f.Create(baseCfg)
	if err != nil {
		t.Fatalf("Create should succeed after enabling ip_forward, got: %v", err)
	}
	if tun == nil {
		t.Fatal("expected non-nil tun file")
	}
...
func TestEnableForwarding_IPv6Skipped_WhenNoIPv6Subnet(t *testing.T) {
	...
	tun, err := f.Create(baseCfg)
	if err != nil {
		t.Fatalf("Create should skip IPv6 forwarding when no IPv6 subnet, got: %v", err)
	}
	_ = tun.Close()
}
...
func TestEnableForwarding_IPv6WritesWhenDisabled_Succeeds(t *testing.T) {
	...
	tun, err := f.Create(baseCfgIPv6)
	if err != nil {
		t.Fatalf("Create should succeed after enabling IPv6 forwarding, got: %v", err)
	}
	_ = tun.Close()
}
...
func TestCreateTunDevice_WithIPv6Subnet_Success(t *testing.T) {
	...
	tun, err := f.Create(cfg)
	if err != nil {
		t.Fatalf("Create with IPv6 subnet should succeed, got: %v", err)
	}
	_ = tun.Close()
}
...
func TestCreateTunDevice_IPv6Only_Success(t *testing.T) {
	...
	tun, err := f.Create(baseCfgIPv6Only)
	if err != nil {
		t.Fatalf("Create with IPv6-only settings should succeed, got: %v", err)
	}
	_ = tun.Close()
}
...
func TestCreateTunDevice_IPv6Only_AddrAddError(t *testing.T) {
	...
	f := newFactory(ipMock, &TunFactoryMockIPT{}, nil, &TunFactoryMockIOCTL{}, &TunFactoryMockSys{})
	_, err := f.Create(baseCfgIPv6Only)
	if err == nil || !strings.Contains(err.Error(), "failed to assign IPv6 to TUN") {
...
func TestCreate_MasqueradeUsesSubnetScopedRules(t *testing.T) {
	ipMock := &TunFactoryMockIP{}
	iptMock := &TunFactoryMockIPT{}
	f := newFactory(ipMock, iptMock, nil, &TunFactoryMockIOCTL{}, &TunFactoryMockSys{})
...
func TestRemove_MasqueradeCleanupUsesSubnetScopedRules(t *testing.T) {
	ipMock := &TunFactoryMockIP{}
	iptMock := &TunFactoryMockIPT{}
	f := newFactory(ipMock, iptMock, nil, &TunFactoryMockIOCTL{}, &TunFactoryMockSys{})
