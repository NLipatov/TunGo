...
func (m runtimeInfoClientManager) Configuration() (*clientconfig.Configuration, error) {
	return m.cfg, m.err
}
...
type runtimeInfoServerManager struct {
	...
	err           error
	configuration func() (*serverconfig.Configuration, error)
	injectErr     error
...
func (m runtimeInfoServerManager) Configuration() (*serverconfig.Configuration, error) {
...
func (m runtimeInfoServerManager) AddAllowedPeer(serverconfig.AllowedPeer) error {
	return nil
}
func (m *runtimeInfoServerManager) ListAllowedPeers() ([]serverconfig.AllowedPeer, error) {
...
func TestServerControlConfiguration(t *testing.T) {
	...
	conf := &serverconfig.Configuration{
		...
		EnableTCP:        true,
		AllowedPeers: []serverconfig.AllowedPeer{{
			Name:      "client-1",
			PublicKey: peerKey,
			Enabled:   true,
			ClientID:  1,
...
func TestClientControlRuntimeInfo_ActiveSettingsError(t *testing.T) {
	control := clientControl{
		manager: runtimeInfoClientManager{
			cfg: &clientconfig.Configuration{Protocol: settings.UNKNOWN},
		},
	}
