...
type clientCreatorFunc func(clientconfig.Configuration, string) error

func (f clientCreatorFunc) Create(cfg clientconfig.Configuration, name string) error {
	return f(cfg, name)
}
...
func makeTestConfig() clientconfig.Configuration {
...
func TestClientControlDelegates(t *testing.T) {
	...
	control := clientControl{
		...
		creator: clientCreatorFunc(func(_ clientconfig.Configuration, name string) error {
			createdName = name
			return nil
		}),
		manager: runtimeInfoClientManager{cfg: &clientconfig.Configuration{}},
	}
...
func TestClientControlCreateFromJSON_InvalidInput(t *testing.T) {
	creatorCalled := false
	control := clientControl{
		creator: clientCreatorFunc(func(clientconfig.Configuration, string) error {
			creatorCalled = true
			return nil
		}),
	}
