...
func setupConns(t testing.TB) (serverConn *net.UDPConn, clientConn *net.UDPConn, clientAdapter io.ReadWriteCloser) {
	t.Helper()
	...
	clientAdapter = NewPeerConn(clientConn, addrPort)
...
func TestUdpAdapter_ReadFastPath(t *testing.T) {
	...
	adapter := clientAdapter.(*peerConn)
