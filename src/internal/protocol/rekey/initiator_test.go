...
func TestMaybeBuildRekeyInit_NilFSM(t *testing.T) {
	s := NewClientRekeyCoordinator(&keys.DefaultKeyDeriver{}, nil, nil, time.Second, time.Now().Add(-time.Hour))
	dst := make([]byte, v1PacketLen)
	_, ok, err := s.MaybeBuildRekeyInit(time.Now(), dst)
	if err != nil || ok {
...
func TestMaybeBuildRekeyInit_BeforeRotateAt(t *testing.T) {
	...
	fsm := rekey.NewStateMachine(rk, []byte("c2s"), []byte("s2c"))
	now := time.Now()
	s := NewClientRekeyCoordinator(&keys.DefaultKeyDeriver{}, fsm, nil, 10*time.Second, now)
	dst := make([]byte, v1PacketLen)
...
func TestMaybeBuildRekeyInit_NotReady(t *testing.T) {
	...
	now := time.Now()
	s := NewClientRekeyCoordinator(&keys.DefaultKeyDeriver{}, fsm, nil, time.Millisecond, now)
...
func TestMaybeBuildRekeyInit_Success(t *testing.T) {
	...
	if len(payload) != v1PacketLen {
		t.Fatalf("expected payload len=%d, got %d", v1PacketLen, len(payload))
	}
	// Verify V1 header.
	if payload[0] != servicepacket.Prefix || payload[1] != servicepacket.VersionV1 || payload[2] != byte(servicepacket.RekeyInit) {
...
func TestMaybeBuildRekeyInit_SchedulesNextAttempt(t *testing.T) {
	...
	s := NewClientRekeyCoordinator(&keys.DefaultKeyDeriver{}, fsm, nil, interval, now)
