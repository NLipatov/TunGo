...
func (c *ClientRekeyCoordinator) MaybeBuildRekeyInit(
	...
	if c.hasPendingPrivateKey {
		...
		securemem.ZeroBytes(privateKey[:])
	}
	if err != nil {
	...
	if err := servicepacket.Encode(servicepacket.RekeyInit, payload); err != nil {
		return nil, false, err
	}
...
func (c *ClientRekeyCoordinator) buildRekeyInitV2Locked(dst []byte) ([]byte, bool, error) {
	...
	if err := servicepacket.Encode(servicepacket.RekeyInitV2, payload); err != nil {
		return nil, false, err
	}
	return payload, true, nil
}
...
func (c *ClientRekeyCoordinator) HandleRekeyAck(
	...
	kind, parsed := servicepacket.Parse(plaindata)
	if !parsed || kind != servicepacket.RekeyAck || len(plaindata) != v1PacketLen || !c.hasPendingPrivateKey {
		return false, nil
	}
	...
	defer securemem.ZeroBytes(newC2S)
	newS2C, err := c.crypto.DeriveKey(shared, currentS2C, deriveLabelS2C)
	if err != nil {
		return false, err
	}
	defer securemem.ZeroBytes(newS2C)
...
func (c *ClientRekeyCoordinator) handleRekeyAckV2Locked(
	...
	if !ok || kind != servicepacket.RekeyAckV2 || len(plaindata) <= serviceHeaderLen || c.pendingV2Init == nil {
		return false, nil
	}
	if carrierEpoch != c.pendingCarrierEpoch {
		return false, nil
...
func (c *ClientRekeyCoordinator) clearPendingPrivateKeyLocked() {
	securemem.ZeroBytes(c.pendingPrivateKey[:])
	c.pendingCarrierEpoch = 0
