...
type peerRekey interface {
	Handle(uint16, keys.KeyDeriver, []byte) ([]byte, uint16, bool, error)
...
func NewPeerWithAuth(
	...
	return newPeerWithAuth(
		...
		newEncryptedSender(writer, crypto),
	)
}
...
func newPeer(
	...
	return newPeerWithAuth(crypto, rekey, internalIP, externalIP, nil, nil, sender)
}
...
func (p *Peer) updateEgressAddr(addr netip.AddrPort) {
	...
	if u, ok := p.sender.(addrPortSetter); ok {
		u.SetAddrPort(addr)
	}
}
