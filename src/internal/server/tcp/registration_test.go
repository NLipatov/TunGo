...
func (*tcpRegHandshake) ClientSideHandshake(_ io.ReadWriter) error {
	return nil
}
func (h *tcpRegHandshake) ServerSideHandshake(_ io.ReadWriter) (int, error) {
	if h.err != nil {
		return 0, h.err
	}
...
func (f *tcpRegHandshakeFactory) NewHandshake() handshake {
	return f.handshake
}
...
func TestNewRegistrar(t *testing.T) {
	r := newRegistrar(nil, nil, nil, netip.MustParsePrefix("10.0.0.0/24"), netip.Prefix{})
	if r == nil {
...
func TestRegisterClient_HandshakeError_ClosesConn(t *testing.T) {
	...
	reg := newRegistrar(hf.NewHandshake, cf.FromHandshake, repo, netip.MustParsePrefix("10.0.0.0/24"), netip.Prefix{})
...
// tcpRegWriter tracks Close calls.
type tcpRegWriter struct {
	closed bool
}

func (*tcpRegWriter) Write(packet []byte) (int, error) { return len(packet), nil }
func (w *tcpRegWriter) Close() error                   { w.closed = true; return nil }
...
func TestRegisterClient_ReplacesExistingSession(t *testing.T) {
	...
	ip := netip.MustParseAddr("10.0.0.2")
	oldWriter := &tcpRegWriter{}
	existingPeer := session.NewPeer(
		tcpRegCrypto{}, nil, ip, netip.MustParseAddrPort("192.168.1.100:9999"), oldWriter,
	)
...
func (h *tcpRegHandshakeWithResult) ClientPubKey() []byte       { return h.result.ClientPubKey() }
func (h *tcpRegHandshakeWithResult) AllowedIPs() []netip.Prefix { return h.result.AllowedIPs() }
...
func (f *tcpRegHandshakeWithResultFactory) NewHandshake() handshake {
	return f.handshake
}
...
func (f *tcpRegCookieHandshakeFactory) NewHandshake() handshake {
	f.calls++
	if f.calls == 1 {
...
func TestRegisterClient_HandshakeWithResult_IPv6(t *testing.T) {
	...
	repo := session.NewRepository()
	reg := newRegistrar(hf.NewHandshake, cf.FromHandshake, repo,
