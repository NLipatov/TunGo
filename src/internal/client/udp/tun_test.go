...
func newTestRekeyCoordinator(controller testRekeyController) *tunnelrekey.ClientRekeyCoordinator {
	return tunnelrekey.NewClientRekeyCoordinator(
		&keys.DefaultKeyDeriver{}, controller, nil, time.Hour, time.Now(),
	)
}

func newDueTestRekeyCoordinator(controller testRekeyController) *tunnelrekey.ClientRekeyCoordinator {
	return tunnelrekey.NewClientRekeyCoordinator(
		&keys.DefaultKeyDeriver{}, controller, nil, time.Millisecond, time.Now().Add(-time.Second),
	)
}

// fakeCrypto implements config.crypto for testing
...
func (w *fakeWriter) packetCount() int {
	w.mu.Lock()
	defer w.mu.Unlock()
	return len(w.data)
}
...
func TestHandleTun_ImmediateCancel(t *testing.T) {
	...
	h := newTunHandler(ctx, &fakeReader{readFunc: func(p []byte) (int, error) {
		...
		return 0, nil
	}}, newPacketSender(w, c), newTestRekeyCoordinator(ctrl),
...
func TestHandleTun_ReadError(t *testing.T) {
	...
	h := newTunHandler(ctx, &fakeReader{readFunc: func(p []byte) (int, error) {
		return 0, errRead
	}}, newPacketSender(&fakeWriter{}, &tunhandlerTestRakeCrypto{}), newTestRekeyCoordinator(ctrl),
...
func TestHandleTun_EncryptError(t *testing.T) {
	...
	h := newTunHandler(ctx, reader, newPacketSender(&fakeWriter{}, &tunhandlerTestRakeCrypto{err: errEnc}), newTestRekeyCoordinator(rekey.NewStateMachine(dummyEpochManager{}, []byte("c2s"), []byte("s2c"))), nil)
...
func TestHandleTun_WriteError(t *testing.T) {
	...
	h := newTunHandler(ctx, reader, newPacketSender(writer, &tunhandlerTestRakeCrypto{prefix: []byte("x:")}), newTestRekeyCoordinator(rekey.NewStateMachine(dummyEpochManager{}, []byte("c2s"), []byte("s2c"))), nil)
...
func TestHandleTun_TransientWriteError_ContinuesLoop(t *testing.T) {
	...
	h := newTunHandler(ctx, reader, newPacketSender(writer, crypto), newTestRekeyCoordinator(ctrl), nil)
	err := h.HandleTun()
...
func TestHandleTun_SuccessThenCancel(t *testing.T) {
	...
	h := newTunHandler(ctx, reader, newPacketSender(writer, crypto), newTestRekeyCoordinator(rekey.NewStateMachine(dummyEpochManager{}, []byte("c2s"), []byte("s2c"))), nil)
...
func TestHandleTun_ReadErrorAfterCancel_ReturnsNil(t *testing.T) {
	...
	ctrl := rekey.NewStateMachine(dummyEpochManager{}, []byte("c2s"), []byte("s2c"))
	h := newTunHandler(ctx, r, newPacketSender(&fakeWriter{}, &tunhandlerTestRakeCrypto{}), newTestRekeyCoordinator(ctrl), nil)
...
func TestHandleTun_EncryptErrorAfterCancel_ReturnsNil(t *testing.T) {
	...
	ctrl := rekey.NewStateMachine(dummyEpochManager{}, []byte("c2s"), []byte("s2c"))
	h := newTunHandler(ctx, r, newPacketSender(&fakeWriter{}, crypt), newTestRekeyCoordinator(ctrl), nil)
...
func TestHandleTun_WriteErrorAfterCancel_ReturnsNil(t *testing.T) {
	...
	ctrl := rekey.NewStateMachine(dummyEpochManager{}, []byte("c2s"), []byte("s2c"))
	h := newTunHandler(ctx, r, newPacketSender(w, &tunhandlerTestRakeCrypto{prefix: []byte("x:")}), newTestRekeyCoordinator(ctrl), nil)
...
func TestHandleTun_ReadReturnsNAndEOF_OneWriteThenEOF(t *testing.T) {
	...
	w := &fakeWriter{}
	ctrl := rekey.NewStateMachine(dummyEpochManager{}, []byte("c2s"), []byte("s2c"))
	h := newTunHandler(ctx, r, newPacketSender(w, &tunhandlerTestRakeCrypto{prefix: []byte("pre-")}), newTestRekeyCoordinator(ctrl), nil)
...
func TestHandleTun_ReusesPendingRekeyKey(t *testing.T) {
	...
	h := newTunHandler(ctx, reader, newPacketSender(writer, crypto), coordinator, nil)
	...
	for writer.packetCount() < 2 && time.Now().Before(deadline) {
...
func TestHandleTun_RekeyInitSendError_DoesNotFail(t *testing.T) {
	...
	coordinator := tunnelrekey.NewClientRekeyCoordinator(
		&keys.DefaultKeyDeriver{}, ctrl, nil, time.Millisecond, time.Now().Add(-time.Second),
	)
...
func TestHandleTun_SourceFilter_DropsNonVPN(t *testing.T) {
	vpnPacket := testIPv4Pkt(netip.MustParseAddr("10.0.0.2"))
	lanPacket := testIPv4Pkt(netip.MustParseAddr("192.168.64.5"))

	callIdx := 0
	packets := [][]byte{lanPacket, vpnPacket}
	reader := &fakeReader{readFunc: func(p []byte) (int, error) {
		if callIdx >= len(packets) {
			return 0, io.EOF
		}
		pkt := packets[callIdx]
		callIdx++
	...
	h := newTunHandler(context.Background(), reader, newPacketSender(writer, crypto), newTestRekeyCoordinator(ctrl), allowed)
	...
	if len(writer.data) != 1 {
		t.Fatalf("writes=%d, want 1 (LAN packet should be dropped)", len(writer.data))
	}
}
