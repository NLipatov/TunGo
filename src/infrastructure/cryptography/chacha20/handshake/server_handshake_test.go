...
// fakeConn implements application.Transport
...
func buildHello(t *testing.T) []byte {
	...
	ch := NewClientHello(ip.V4, net.ParseIP("10.0.0.4"), edPub, make([]byte, curvePublicKeyLength), make([]byte, nonceLength), packet_validation.NewDefaultIPValidator(packet_validation.Policy{
		AllowV4:           true,
		AllowV6:           true,
