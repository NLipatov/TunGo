...
type crypto interface {
	...
	GenerateChaCha20Keys(
		curvePrivate,
		serverNonce []byte,
		extHello ClientHello) (sessionId [32]byte, serverToClientKey, clientToServerKey []byte, err error)
}
...
func (c *defaultCrypto) GenerateChaCha20Keys(
	curvePrivate,
	serverNonce []byte,
	extHello ClientHello) (sessionId [32]byte, serverToClientKey, clientToServerKey []byte, err error) {
	// Generate shared secret and salt
	sharedSecret, _ := curve25519.X25519(curvePrivate[:], extHello.curvePublicKey)
	salt := sha256.Sum256(append(serverNonce, extHello.clientNonce...))
	...
	return sessionId, serverToClientKey, clientToServerKey, nil
}
