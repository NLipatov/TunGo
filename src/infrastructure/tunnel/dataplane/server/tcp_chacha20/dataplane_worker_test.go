...
func (c *dpwTestCrypto) Decrypt(_ []byte) ([]byte, error) {
	c.decryptCalls++
	return c.plaintext, nil
}

type dpwFailingEgress struct{}

func (dpwFailingEgress) SendDataIP([]byte) error  { return nil }
func (dpwFailingEgress) SendControl([]byte) error { return errors.New("send failed") }
func (dpwFailingEgress) Close() error             { return nil }
...
func TestTCPDataplaneWorker_Run_StopsAfterRekeyAckSendError(t *testing.T) {
	deriver := &primitives.DefaultKeyDeriver{}
	initPacket := buildTCPRekeyInitPacket(t, deriver)
	crypto := &dpwTestCrypto{plaintext: initPacket}
	fsm := rekey.NewStateMachine(&tcpTestEpochManager{}, make([]byte, 32), make([]byte, 32))
	coordinator := controlplane.NewServerRekeyCoordinator(fsm)
	internal := netip.MustParseAddr("10.0.0.1")
	sess := session.NewSession(crypto, coordinator, internal, netip.MustParseAddrPort("203.0.113.10:41000"))
	peer := session.NewPeer(sess, dpwFailingEgress{})
	repo := &fakeSessionRepo{}

	worker := newDPWWorker(
		peer,
		&fakeConn{readBufs: [][]byte{make([]byte, 32), make([]byte, 32)}},
		&fakeWriter{},
		repo,
	)
	worker.Run()
	...
	if crypto.decryptCalls != 1 {
		t.Fatalf("expected worker to stop after first failed ACK, decrypt calls=%d", crypto.decryptCalls)
	}
}
