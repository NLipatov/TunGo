...
var (
	...
	uiStylesCache        = map[ThemeOption]uiStyles{}
)
...
func pickByTheme(prefs UIPreferences, light, dark string) string {
...
func ansiStylePrefix(fgHex, bgHex string, bold bool) string {
	rFg, gFg, bFg, okFg := parseHexColor(fgHex)
	rBg, gBg, bBg, okBg := parseHexColor(bgHex)
	if !okFg || !okBg {
		return ""
	}
	var out strings.Builder
	...
	out.WriteString(strconv.Itoa(rFg))
	out.WriteByte(';')
	out.WriteString(strconv.Itoa(gFg))
	out.WriteByte(';')
	out.WriteString(strconv.Itoa(bFg))
	out.WriteByte('m')
	out.WriteString("\x1b[48;2;")
	out.WriteString(strconv.Itoa(rBg))
	out.WriteByte(';')
	out.WriteString(strconv.Itoa(gBg))
	out.WriteByte(';')
	out.WriteString(strconv.Itoa(bBg))
	out.WriteByte('m')
	return out.String()
}
...
func parseHexColor(hex string) (r, g, b int, ok bool) {
	...
	vr, errR := strconv.ParseUint(hex[1:3], 16, 8)
	vg, errG := strconv.ParseUint(hex[3:5], 16, 8)
	vb, errB := strconv.ParseUint(hex[5:7], 16, 8)
	if errR != nil || errG != nil || errB != nil {
		return 0, 0, 0, false
	}
	return int(vr), int(vg), int(vb), true
}
...
func resolveUIStyles(prefs UIPreferences) uiStyles {
	...
	textColor := pickByTheme(prefs, p.textLight, p.textDark)
	mutedColor := pickByTheme(prefs, p.mutedLight, p.mutedDark)
	accentColor := p.accent
	activeTextColor := p.activeText
	backgroundColor := pickByTheme(prefs, p.backgroundLight, p.backgroundDark)

	textPrefix := ansiStylePrefix(textColor, backgroundColor, false)
	mutedPrefix := ansiStylePrefix(mutedColor, backgroundColor, false)
	accentPrefix := ansiStylePrefix(accentColor, backgroundColor, true)
	rulePrefix := ansiStylePrefix(accentColor, backgroundColor, false)
	activePrefix := ansiStylePrefix(activeTextColor, accentColor, true)

	styles := uiStyles{
		headerBar:   ansiTextStyle{prefix: textPrefix},
		brand:       ansiTextStyle{prefix: accentPrefix},
