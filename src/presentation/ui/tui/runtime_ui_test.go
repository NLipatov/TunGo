...
type runtimeUITestBackend struct {
	enabledCap int
	disabled   bool
	run        func(ctx context.Context, mode runtime.Mode, options RuntimeUIOptions) (bool, error)
}
...
func (m *runtimeUITestBackend) runRuntimeDashboard(ctx context.Context, mode runtime.Mode, options RuntimeUIOptions) (bool, error) {
	if m.run == nil {
		return false, nil
	}
...
func (m *runtimeUITestBackend) setSessionHolder(*sessionHolder) {}

func TestRuntimeUI_Wrappers(t *testing.T) {
	mock := &runtimeUITestBackend{}
	runtimeUI := newRuntimeUI(mock)

	runtimeUI.EnableRuntimeLogCapture(42)
	...
	mock.run = func(_ context.Context, mode runtime.Mode, options RuntimeUIOptions) (bool, error) {
		if mode != runtime.ModeServer {
			t.Fatalf("expected server mode mapping, got %v", mode)
		}
	...
	quit, err := runtimeUI.RunRuntimeDashboard(context.Background(), runtime.ModeServer, RuntimeUIOptions{
	...
	mock.run = func(_ context.Context, mode runtime.Mode, _ RuntimeUIOptions) (bool, error) {
		if mode != runtime.ModeClient {
			t.Fatalf("expected client mode mapping, got %v", mode)
		}
		return false, errors.New("boom")
	}
	quit, err = runtimeUI.RunRuntimeDashboard(context.Background(), runtime.ModeClient, RuntimeUIOptions{})
	if err == nil || quit {
