...
type configuratorSessionModel struct {
	...
	pendingStartMode    command.Command
	pendingStartScreen  configuratorScreen
	pendingClientConfig string
	pendingDaemonMode   command.Command

	resultMode command.Command
	resultErr  error
	done       bool
}

func RunConfiguratorSession(options ConfiguratorSessionOptions) (selectedMode command.Command, err error) {
	...
	if runErr != nil {
		return command.Unknown, runErr
	}

	result, ok := finalModel.(configuratorSessionModel)
	if !ok {
		return command.Unknown, errors.New("invalid configurator session model")
	}
	if result.resultErr != nil {
		return command.Unknown, result.resultErr
	}
					...
					if err := model.options.Selector.Select(autoConfig); err == nil {
						...
						if model.options.ClientConfigManager != nil {
							...
							if isInvalidClientConfigurationError(cfgErr) {
								...
								model = model.startModeWithSystemdGuard(command.StartClient, configuratorScreenClientSelect, true)
...
func (m configuratorSessionModel) updateClientSelectScreen(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	default:
		...
		m = m.startModeWithSystemdGuard(command.StartClient, configuratorScreenClientSelect, false)
		if m.done {
...
func (m configuratorSessionModel) updateServerSelectScreen(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case sessionServerStart:
		m = m.startModeWithSystemdGuard(command.StartServer, configuratorScreenServerSelect, false)
		if m.done {
			return m, tea.Quit
		}
		return m, nil
...
func (m configuratorSessionModel) updateDaemonManageScreen(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case sessionDaemonSetupClient:
		m, err = m.applyDaemonSetup(command.StartClient, false)
		if err != nil {
			m.notice = err.Error()
			return m, nil
		}
	case sessionDaemonSetupServer:
		m, err = m.applyDaemonSetup(command.StartServer, false)
		if err != nil {
			m.notice = err.Error()
			return m, nil
		}
	case sessionDaemonReconfClient:
		if daemonStateBlocksRuntimeStart(m.daemon.status.ActiveState) {
			m.pendingDaemonMode = command.StartClient
			m.cursor = 0
	...
	case sessionDaemonReconfServer:
		if daemonStateBlocksRuntimeStart(m.daemon.status.ActiveState) {
			m.pendingDaemonMode = command.StartServer
			m.cursor = 0
			m.screen = configuratorScreenDaemonReconfigureConfirm
			return m, nil
		}
...
func (m configuratorSessionModel) updateDaemonReconfigureConfirmScreen(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case "esc":
		m.screen = configuratorScreenDaemonManage
		m.cursor = 0
		m.pendingDaemonMode = command.Unknown
...
func (m configuratorSessionModel) applyDaemonSetup(targetMode command.Command, restartRunning bool) (configuratorSessionModel, error) {
	switch targetMode {
	case command.StartClient:
...
func (m configuratorSessionModel) updateSystemdCheckErrorConfirmScreen(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	case sessionRetrySystemdCheck:
		...
		if !m.done && isSystemdStartConfirmationScreen(m.screen) && targetMode == command.StartClient {
...
func (m configuratorSessionModel) completePendingSystemdStart(notice string) (configuratorSessionModel, tea.Cmd) {
	...
	if targetMode == command.StartClient {
...
func (m configuratorSessionModel) startModeWithSystemdGuard(targetMode command.Command, returnScreen configuratorScreen, preserveNotice bool) configuratorSessionModel {
	m = m.clearPendingSystemdStart()
...
func (m configuratorSessionModel) clearPendingSystemdStart() configuratorSessionModel {
	m.pendingStartMode = command.Unknown
