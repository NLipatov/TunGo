...
func (c *clientConfigurator) configureFromState(state clientFlowState) error {
	...
	for {
		...
		case clientStateAddName:
			...
			normalizedConfigurationName := strings.TrimSpace(textInputValue)
			if normalizedConfigurationName == "" {
				if warningErr := c.showEmptyConfigurationNameWarning(); warningErr != nil {
					return warningErr
				}
				flow.state = clientStateAddName
				continue
			}
			flow.newConfigurationName = normalizedConfigurationName
...
func (c *clientConfigurator) showEmptyConfigurationNameWarning() error {
	selectedOption, selectErr := c.selectConf(
		[]string{invalidConfigOkOption},
	...
	if selectedOption != invalidConfigOkOption {
		return fmt.Errorf("configuration selection aborted")
	}
	return nil
}
