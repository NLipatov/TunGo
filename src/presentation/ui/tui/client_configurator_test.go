...
type stagedTextInputMock struct {
	values []string
	errs   []error
	idx    int
}

func (m *stagedTextInputMock) Value() (string, error) {
...
func Test_Configure_AddOption_EmptyName_ShowsWarningAndRetriesNameInput(t *testing.T) {
	obs := &cfgObserverMock{
		results: [][]string{
			{},
			{"newconf.json"},
	...
	sf := &queuedSelectorFactory{
		selector: &queuedSelector{options: []string{addOption, invalidConfigOkOption, "newconf.json"}},
	}
	tif := &textInputFactoryMock{ti: &stagedTextInputMock{
		values: []string{"   ", "newconf"},
	}}
	validCfgJSON, _ := json.Marshal(makeTestConfig())
	taf := &textAreaFactoryMock{ta: &textAreaMock{val: string(validCfgJSON)}}
	creator := &cfgCreatorMock{}
