...
func TestTransportConnectorImplementsConnector(t *testing.T) {
	var _ application.Connector = &transport_connector.TransportConnector{}
}
...
func TestEstablishConnectionWithRetry_Success(t *testing.T) {
	mockSession := &chacha20.TcpCryptographyService{}
	mockConn := &MockConn{}

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	connectorDelegate := func() (net.Conn, application.CryptographyService, error) {
...
func TestEstablishConnectionWithRetry_RetryAndFail(t *testing.T) {
	...
	if cryptographyService != nil {
		t.Fatalf("expected nil cryptographyService, got %v", cryptographyService)
	}
