...
func TestRuntimeAddressInfoFromServerConfiguration_CollectsAllEnabledTunnelAddresses(t *testing.T) {
	...
	if got.TunnelAddresses[0].Protocol != settings.TCP || got.TunnelAddresses[0].IPv4 != netip.MustParseAddr("10.0.0.1") || got.TunnelAddresses[0].IPv6 != netip.MustParseAddr("fd00::1") {
		t.Fatalf("unexpected TCP tunnel address entry: %+v", got.TunnelAddresses[0])
	}
	if got.TunnelAddresses[1].Protocol != settings.UDP || got.TunnelAddresses[1].IPv4 != netip.MustParseAddr("10.0.1.1") || got.TunnelAddresses[1].IPv6 != netip.MustParseAddr("fd00::2") {
		t.Fatalf("unexpected UDP tunnel address entry: %+v", got.TunnelAddresses[1])
	}
	if got.TunnelAddresses[2].Protocol != settings.WS || got.TunnelAddresses[2].IPv4 != netip.MustParseAddr("10.0.2.1") || got.TunnelAddresses[2].IPv6 != netip.MustParseAddr("fd00::3") {
