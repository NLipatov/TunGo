...
func TestRun_Interactive_ReconfigureReturnsBackToModeSelection(t *testing.T) {
	...
	r := newTestServerRunner(deps, wf, rf, func(context.Context, tui.RuntimeMode, tui.RuntimeUIOptions) (bool, error) {
		return true, nil
	})
...
func TestRun_Interactive_WorkerErrorWinsOverUIError(t *testing.T) {
	...
	r := newTestServerRunner(deps, wf, rf, func(ctx context.Context, _ tui.RuntimeMode, _ tui.RuntimeUIOptions) (bool, error) {
		<-ctx.Done()
		return false, errors.New("ui failed")
	})
	err := r.Run(context.Background())
