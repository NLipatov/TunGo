...
import (
	...
	"os"
	"sync"
	pal_factory2 "tungo/infrastructure/PAL/pal_factory"
...
func (r *Runner) route(ctx context.Context, settings settings.ConnectionSettings) error {
	workerFactory := pal_factory2.NewServerWorkerFactory(settings)
	tunFactory := pal_factory2.NewServerTunFactory()
