...
import (
	...
	"os"
	"sync"
	server2 "tungo/Application/server"
...
func StartTCPRouting(tunFile *os.File, settings settings.ConnectionSettings) error {
	...
	go func() {
		defer wg.Done()
		server2.TunToTCP(tunFile, &extToLocalIp, &extIpToSession, ctx)
	}()

	// TCP -> TUN
	go func() {
		defer wg.Done()
		server2.TCPToTun(settings, tunFile, &extToLocalIp, &extIpToSession, ctx)
	}()
...
func StartUDPRouting(tunFile *os.File, settings settings.ConnectionSettings) error {
	...
	go func() {
		defer wg.Done()
		server2.TunToUDP(tunFile, &intIPToUDPClient, &intIPToSession, ctx)
	}()

	// UDP -> TUN
	go func() {
		defer wg.Done()
		server2.UDPToTun(settings, tunFile, &intIPToUDPClient, &intIPToSession, ctx)
	}()
